...
export class SearchProvider {
  ...
  private inputTypes: InputType[] = [
    ...
    // Standard BTE
    {
      regexes: [/^(widecoin:)?([ED][a-km-zA-HJ-NP-Z1-9]{25,34})/],
      dataIndex: 2,
      type: 'address',
      chainNetworks: [
    ...
    // bech32 BTE Address
    {
      regexes: [/^(bitweb:)?(web[ac-hj-np-zAC-HJ-NP-Z02-9]{11,71})/],
      dataIndex: 2,
      type: 'address',
      chainNetworks: [
        { chain: 'BTE', network: 'mainnet' }
    ...
    {
      regexes: [/^(bitcoin:|bchtest:|dogecoin:|bitweb:)?([2mn][1-9A-HJ-NP-Za-km-z]{26,35})/],
      dataIndex: 2,
      type: 'address',
      chainNetworks: [
  ...
  private extractAddress(address: string): string {
    const extractedAddress = address
      .replace(/^(bitcoincash:|bchtest:|bitcoin:|bitweb:)/i, '')
      .replace(/\?.*/, '');
