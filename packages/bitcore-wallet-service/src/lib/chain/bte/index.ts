...
export class BteChain extends BtcChain implements IChain {
  constructor(private bitcoreLibBte = BitcoreLibBte) {
    super(BitcoreLibBte);
  }

  selectTxInputs(server, txp, wallet, opts, cb) {
    const MAX_TX_SIZE_IN_KB = Defaults.MAX_TX_SIZE_IN_KB_BTE;
    ...
    const select = (utxos, coin, cb) => {
      ...
      _.each(smallInputs, (input, i) => {
        ...
        if (netTotal >= txpAmount) {
          ...
          const dustThreshold = Math.max(Defaults.MIN_OUTPUT_AMOUNT, this.bitcoreLibBte.Transaction.DUST_AMOUNT);
